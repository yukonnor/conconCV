...
void loop() { 
  ...
  pot_3 = analogRead(A2);
    LFO_B_rate = map(pot_3, 0, 1023, 1, 200); // 1-100 microsecords
  pot_4= analogRead(A3);
  ...
  bool buttonRead1 = digitalRead(button1Pin); //BUTTON1 Seems to be stuck 'down'
  bool buttonRead2 = digitalRead(button2Pin);
  //Serial.print("    Button 1 State = ");
  //Serial.print(buttonRead1);  //BUTTON1 Seems to be stuck 'down'
  ...
  if((current_time - time_A_previous) > LFO_A_rate){                    //once a certain amount of time passes, advance the table step and write the value
    ...
    /*
    if(debug == 1){
      Serial.print("LFO A Rate = ");   //pot A value
      Serial.print(pot_1); 
      Serial.print("      LFO A Offset = ");  //pot A' value
      Serial.print(pot_2);
      ...
      Serial.println(waveTable_255[shapeA][tableStepA]);
      } 
      */
